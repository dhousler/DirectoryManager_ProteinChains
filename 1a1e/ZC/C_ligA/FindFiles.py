...
def GetPAPContacts():
    ...
    os.chdir(path)
    contact_file = [f for f in os.listdir(path) if f.endswith('.csv')]
    contact_file_str = str(contact_file)
    contact_file_str = re.sub('[\'\[\]]','',contact_file_str)
    print(contact_file_str)
...
def GetWaterContacts():
    path = start_directory + '/proACT_run/'
    os.chdir(path)
    water_contact_file = [f for f in os.listdir(path) if f.endswith('.csv')]
...
P_AP_contacts = GetPAPContacts()
water_contacts = GetWaterContacts()
...
if os.stat(save_f).st_size==0: #Checks if file is empty
    ...
    g.write(pdb_ref + ','  + chain_ref  + ',' + chain_ref  + '_' + ligand_ref + ',' + P_AP_contacts + ',' + water_contacts + ',' + str(TOTAL_contacts) + ',')
    g.write('\n')
    g.close()#Closes PDB File
else:
    g = open(save_f, 'a+')
